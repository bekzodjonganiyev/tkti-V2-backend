...
class MediaController {
  async Add(req, res) {
    try {
      const { error, value } = validate.postMediaValidation.validate({
        ...req.body,
      });
      if (error) {
        res
  ...
  async Edit(req, res) {
    try {
      ...
      const updated = await Media_data.findByIdAndUpdate(
        req.params.id,
        { ...value },
        { new: true }
      );
  ...
  async RemoveCasheFiles(_, res) {
    try {
      ...
      res.status(200).json({
        status: 200,
        success: true,
        message: `Yaxshi ${result.length}ta musor fayl bor `,
        data: result,
      });
    } catch (e) {
      console.log(e);
