...
class RektoratController{
    async Add(req, res) {
        try{
            ...
            res.status(200).json({status:200,success:true, message:`Yangi data qo'shildi`, data: Rektorat})
        }
        catch(e){
            if(req.file){
                removeMedia(req.file.filename)
    ...
    async Edit(req, res) {
        try{
            ...
            const updated = await Rektorat_data.findByIdAndUpdate(req.params.id, {...value}, {new:true});

            res.status(200).json({status:200,success:true, message:`data yangilandi`, data: updated})
        }
        catch(e){
            if(req.file){
                removeMedia(req.file.filename)
    ...
    async Get(_, res) {
        try{
            ...
            res.status(200).json({status:200,success:true, message:`Yaxshi uka`, data: Rektorat})
        }
        catch(e){
            console.log(e);
            res.status(500).json({status:500, message:'invalid request', success:false});
    ...
    async GetById(req, res) {
        try{
            ...
            const Rektorat = await  Rektorat_data.aggregate( [
               ...
               {$lookup:{ from: "faoliyats", localField: "_id", foreignField: "rektorat_id", as: "faoliyatlar"}}
             ] );
            if(!Rektorat){
                res.status(404).json({status:404, message:'data topilmadi', success:false});
                return
            }
            res.status(200).json({status:200,success:true, message:`Yaxshi uka`, data: Rektorat[0]})
        }
        catch(e){
            console.log(e);
            res.status(500).json({status:500, message:'invalid request', success:false});
    ...
    async Delete(req, res) {
        try{            
            ...
            const RektoratFaoliyat = await Faoliyat_data.find({rektorat_id: req.params.id});
            ...
            res.status(200).json({status:200,success:true, message:`Yaxshi  delet qilindi`, data: Rektorat})
        }
        catch(e){
            console.log(e);
            res.status(500).json({status:500, message:'invalid request', success:false});
