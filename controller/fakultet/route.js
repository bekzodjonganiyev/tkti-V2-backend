      ...
      if (err || !req.file) {
        res
          ...
          .json({
            status: 404,
            success: false,
            message: `Fakultetga hodim qo'shish uchun 'photo' keyidan foydalanib 1ta rasm yuboring yuboring`,
          });
        return;
      }
      next();
...
FakultetHodimRouter.get(
  "/all",
  middleware.checkContentType,
  middleware.checkToken,
  FakultetHodimController.Get
);
FakultetHodimRouter.get(
  "/:id",
  middleware.checkParamsId,
  middleware.checkContentType,
  FakultetHodimController.GetById
);
FakultetHodimRouter.put(
  "/:id",
  middleware.checkParamsId,
  middleware.checkToken,
  (req, res, next) => {
...
FakultetHodimRouter.delete(
  "/:id",
  middleware.checkParamsId,
  middleware.checkContentType,
  middleware.checkToken,
  FakultetHodimController.Delete
);
...
FakultetRouter.put(
  ...
  middleware.checkContentType,
  middleware.checkToken,
  FakultetController.Edit
);
FakultetRouter.delete(
  "/:id",
  middleware.checkParamsId,
  middleware.checkContentType,
  middleware.checkToken,
  FakultetController.Delete
);
