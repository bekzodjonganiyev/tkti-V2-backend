const AboutUsData = require("./model");
...
class AboutUsController {
  async Add(req, res) {
    try {
      ...
      const AboutUs = new AboutUsData({...obj, link: images});
      await AboutUs.save();
  ...
  async Edit(req, res) {
    try {
      ...
      const updated = await AboutUsData.findByIdAndUpdate(
        req.params.id,
        { ...value },
        { new: true }
      );
  ...
  async Get(_, res) {
    try {
      const About = await AboutUsData.find();
  ...
  async GetById(req, res) {
    try {
      const Media = await AboutUsData.findOne({ _id: req.params.id });
  ...
  async Delete(req, res) {
    try {
      const AboutUs = await AboutUsData.findByIdAndDelete(req.params.id);
      if (!AboutUs) {
